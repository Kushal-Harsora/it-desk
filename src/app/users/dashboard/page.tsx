...
export default function Page() {
                                    ...
                                    <div className=" w-full h-fit flex flex-col justify-center items-center gap-2">
                                        ...
                                        {row_data?.attachment && row_data?.id && (
                                            <div className="flex flex-col justify-center items-center gap-2">
                                                ...
                                                <a
                                                    href={`/api/user/tickets/${row_data.id}/attachment`}
                                                    target="_blank"
    ...
    React.useEffect(() => {
        const getData = async () => {
            ...
            try {
                ...
                const response_ticket: AxiosResponse = await axios.get(`/api/user/tickets?name=${name}&email=${email}&start=${toZonedTime(new Date('1900-01-01T00:00:00Z'), timeZone)}&end=${toZonedTime(new Date(), timeZone)}`);
    ...
    const onSubmit = async (values: z.infer<typeof TicketSchema>) => {
        ...
        try {
            const response: AxiosResponse = await axios.post('/api/user/tickets', formData, {
                headers: {
    ...
    const onSubmitCalendar = async (data: { dateRange: { from: Date; to: Date } }) => {
        try {
            ...
            const res = await axios.get(`/api/user/tickets?name=${name}&email=${email}&start=${from}&end=${to}`);
